...
function selectButtonNav() {
    ...
    else if(page === "q_a" || page === "q_a_thread") document.querySelector("body header nav #navbarSupportedContent a#q_a_button").style.color = "#2286DB"
...
let tags = document.querySelectorAll(".btn.btn-light.btn-sm.tag span")
tags.forEach(tag => {
...
/**
 * Close messages displayed
 */

let close_buttons_message = document.querySelectorAll("div.messages div.close_button i")
for (let button of close_buttons_message) button.addEventListener('click', closeMessage)

function closeMessage(event) {
    let message = event.target.parentNode.parentNode.parentNode;
    message.remove()
}
